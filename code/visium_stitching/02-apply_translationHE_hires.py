...
sample_info_path = here('processed-data', 'visium_stitching', 'transformationsHE.csv')

combined_dir = Path(here('processed-data', 'visium_stitching', f'combined_{this_donor}'))
combined_dir.mkdir(parents = True, exist_ok = True)

img_out_fullres = Path(here(combined_dir, f'combined_{this_donor}.tif'))
img_out_lowres = Path(here(combined_dir, f'combined_{this_donor}_lowres.tif'))
...
def merge_image_fullres(sample_info, theta, trans_img, max0, max1):
    ...
    #   image
    combined_img = np.full((max0 + 1, max1 + 1, 3, sample_info.shape[0]), 255, dtype = np.uint8)
    ...
    combined_img = np.min(combined_img, axis = 3).astype(np.uint8)
    return combined_img
    
def merge_image_lowres(sample_info, theta, trans_img, max0, max1):
    lowres_sf = LOWRES_MAX_SIZE / max(max0, max1)
        #   Rescale relevant variables to reflect pixels in low resolution
    scaled_trans_img = (trans_img * lowres_sf).astype(int)
    max0 = round(max0 * lowres_sf)
    max1 = round(max1 * lowres_sf)
    #combined_img = np.full((max0 + 2, max1 + 2, 3, sample_info.shape[0]), 255, dtype = np.uint8)
    combined_img = np.zeros((max0 + 2, max1 + 2, 3), dtype = int)
    weights = np.zeros((max0 + 2, max1 + 2, 1), dtype = int)
    for i in range(sample_info.shape[0]):
        ...
        #   Rotate about the top left corner of the image
        theta_deg = 180 * theta[i] / np.pi # '.rotate' uses degrees, not radians
        img = np.array(img_pil.rotate(theta_deg, expand = True, fillcolor = BACKGROUND_COLOR),dtype = int)
        #   "Place this image" on the combined image, considering translations.
        #combined_img[scaled_trans_img[i, 0]: scaled_trans_img[i, 0] + img.shape[0], scaled_trans_img[i, 1]: scaled_trans_img[i, 1] + img.shape[1] ,:, i] = img
        combined_img[scaled_trans_img[i, 0]: scaled_trans_img[i, 0] + img.shape[0], scaled_trans_img[i, 1]: scaled_trans_img[i, 1] + img.shape[1] ,:] += img
        weights[scaled_trans_img[i, 0]: scaled_trans_img[i, 0] + img.shape[0], scaled_trans_img[i, 1]: scaled_trans_img[i, 1] + img.shape[1],:] += 1
...
def merge_image_hires(sample_info, theta, trans_img, max0, max1):
    hires_sf1 = sample_info['hires_sf'].iloc[0]
        #   Rescale relevant variables to reflect pixels in low resolution
    scaled_trans_img = (trans_img * hires_sf1).astype(int)
    max0 = round(max0 * hires_sf1)
    max1 = round(max1 * hires_sf1)
    #combined_img = np.full((max0 + 2, max1 + 2, 3, sample_info.shape[0]), 255, dtype = np.uint8)
    combined_img = np.zeros((max0 + 2, max1 + 2, 3), dtype = int)
    weights = np.zeros((max0 + 2, max1 + 2, 1), dtype = int)
    for i in range(sample_info.shape[0]):
        #   Read in full-res RGB image and scale to low res
        img_pil = Image.open(sample_info['raw_image_path'].iloc[i])
        hires_sf = sample_info['hires_sf'].iloc[i]
        scaled_size = tuple([round(x * hires_sf) for x in img_pil.size])
        img_pil = img_pil.resize(scaled_size)
        #   Rotate about the top left corner of the image
        theta_deg = 180 * theta[i] / np.pi # '.rotate' uses degrees, not radians
        img = np.array(img_pil.rotate(theta_deg, expand = True, fillcolor = BACKGROUND_COLOR),dtype = int)
...
combined_img = merge_image_lowres(sample_info, theta, trans_img, max0, max1)
with tifffile.TiffWriter(img_out_lowres, bigtiff = True) as tiff:
    tiff.write(combined_img)

combined_img = merge_image_hires(sample_info, theta, trans_img, max0, max1)
with tifffile.TiffWriter(img_out_lowres, bigtiff = True) as tiff:
    tiff.write(combined_img)
