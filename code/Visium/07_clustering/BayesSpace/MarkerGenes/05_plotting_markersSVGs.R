...
spe <- readRDS(here("processed-data","Visium", "06_batch_correction", "spe_harmony_markersSVGs.rds"))
dim(spe)

# get folders in cluster_Csv
bs_folders <- list.files(here::here("processed-data","Visium", "07_clustering", "BayesSpace","MarkerGenes","cluster_csv_markersSVGs"), full.names = TRUE)
bs_folders
...
for (i in seq_along(bs_folders)) {
    ...
    pdf(file = here::here("plots", "Visium", "07_clustering", "BayesSpace", "MarkerGenes", "markersSVGs", paste0("BS_k", bs_k[i], ".pdf")), width = 5 * length(unique(spe$sample_id)), height = 5)
