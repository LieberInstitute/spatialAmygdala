...
for (i in seq_along(bs_folders)) {
    ...
    pdf(file = here::here("plots", "Visium", "07_clustering", "BayesSpace","HVGs", "Br8325", paste0("BS_k", bs_k[i], "_stitched.pdf")), width = 10, height = 10)
...
for (i in seq_along(bs_folders)) {
    ...
    pdf(file = here::here("plots", "Visium", "07_clustering", "BayesSpace","HVGs", "Br8325", paste0("LIBD_BS_k", bs_k[i], "_stitched_histo.pdf")), width = 10, height = 10)
    p <- vis_clus(
        spe.test,
        clustervar=clustV,
        colors = pal,
