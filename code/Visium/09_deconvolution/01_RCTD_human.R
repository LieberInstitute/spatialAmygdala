...
# load
sce <- readRDS(here("processed-data", "snRNAseq", "sce_amy_human.rds"))
sce
...
cell_type_counts <- table(sce$fine_celltype)
keep_types <- names(cell_type_counts)[cell_type_counts >= 25]
sce <- sce[, sce$fine_celltype %in% keep_types]
...
## Cell type labels — adjust "fine_celltype" to match your colData column
