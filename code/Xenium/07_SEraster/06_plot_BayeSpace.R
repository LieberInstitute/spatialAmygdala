...
spe <- readRDS(here("processed-data", "Xenium", "07_SEraster", "spe_rasterized_harmony_259.rds"))
dim(spe)
...
bs_folders <- list.files(here::here("processed-data","Xenium", "07_SEraster", "BayesSpace"), full.names = TRUE)
bs_folders
...
for (i in seq_along(bs_folders)) {
    ...
    pdf(file = here::here("plots", "Xenium", "07_SEraster", "BayesSpace", paste0("BS_k", bs_k[i], "_stitched.pdf")), width = 5, height = 5)
    for (sample_id in unique(colData(spe)$sample_id)) {
        spe.subset <- spe[, colData(spe)$sample_id == sample_id]
        p <- make_escheR(spe.subset) |>
            add_fill(var=clustV, point_size=2) +
...
for (i in seq_along(bs_folders)) {
    ...
    pdf(file = here::here("plots", "Xenium", "07_SEraster", "BayesSpace", paste0("LIBD_BS_k", bs_k[i], "_stitched_histo.pdf")), width = 10, height = 10)
    p <- vis_clus(
        spe.test,
        clustervar=clustV,
        colors = pal,
