...
# subset to bad samples - Br9017, Br9206
spe <- spe[, spe$brnum %in% c("Br9017", "Br9206")]
...
if (all(c("cell_area", "nucleus_area") %in% cd_names)) {
    ...
    # QC plots
    png(file = file.path(plot_dir, paste0(obj_name, "_badSamples_cell_area_scaling_factors.png")),
        width = 10, height = 5, units = "in", res = 300)
    hist(spe$cell_area.sf, breaks = 50, main = "Cell area scaling factor", xlab = "Scaling factor")
    dev.off()

    png(file = file.path(plot_dir, paste0(obj_name, "_badSamples_nucleus_area_scaling_factors.png")),
        width = 10, height = 5, units = "in", res = 300)
...
# PCA plots
png(file.path(plot_dir, paste0(obj_name, "_badSamples_Uncorrected_PCs_Brnum.png")), width = 8, height = 6, units = "in", res = 300)
plotReducedDim(spe, dimred = "PCA", ncomponents = 4, colour_by = "brnum", scattermore=TRUE)
dev.off()

png(file.path(plot_dir, paste0(obj_name, "_badSamples_Corrected_PCs_Brnum.png")), width = 8, height = 6, units = "in", res = 300)
plotReducedDim(spe, dimred = "HARMONY", ncomponents = 4, colour_by = "brnum", scattermore=TRUE)
dev.off()

# UMAP plot
png(file.path(plot_dir, paste0(obj_name, "_badSamples_Corrected_UMAP_Brnum.png")), width = 10, height = 10, units = "in", res = 300)
...
png(file.path(plot_dir, paste0(obj_name, "_badSamples_Corrected_UMAP_markers.png")), width = 20, height = 10, units = "in", res = 300)
cowplot::plot_grid(plotlist = plots, ncol = 4)
dev.off()
# Save
message("Saving: ", obj_name)
output_file <- file.path(processed_dir, paste0(obj_name, "_badSamples_harmonized_singlecell.rds"))
